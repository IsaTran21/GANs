...
from utils import select_img, visualize_imgs

def training_step(batch_size=BATCH_SIZE, epochs=EPOCHS, realA_path="./data/trainA", realB_path="./data/trainB", use_transformA=True, use_transformB=True):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    lambda_val = {'gan':GAN, 'cycle':CYCLE, 'iden':IDEN}
    # Create the models
    ...
    train_loader = get_dataloader(realA_path, realB_path, batch_size, use_transformA=use_transformA, use_transformB=use_transformB)
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (imA, imB) in enumerate(train_loader):
            ...
            with torch.autocast(device_type=device.type, dtype=torch.bfloat16):
                gen_loss_val = total_gen_loss(genA, genB, discA, discB, imA, imB)
